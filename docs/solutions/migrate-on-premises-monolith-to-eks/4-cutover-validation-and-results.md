...
This phase separates the claims that a starting container, a passing pipeline,
and a Ready workload each support, because none of them amounts to migration
acceptance. It defines the test strategy, functional parity contract, traffic
cutover, abort thresholds, rollback sequence, and observation window, then
states which results are retained and which remain unverified.
