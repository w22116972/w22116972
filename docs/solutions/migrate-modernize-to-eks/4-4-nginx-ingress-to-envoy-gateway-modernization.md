...
**Priority topic:** 23
**Evidence status:** Gateway API and Envoy Gateway mechanisms are supported by
retained architecture and operational evidence. Each migration still requires
route, policy, traffic, and legacy-load-balancer validation.
...
1. Inventory every Ingress, annotation, class, controller ConfigMap, certificate,
   DNS record, external load balancer, client pinned to an ELB hostname, and
   policy behavior.
...
4. Run direct endpoint tests for routing, TLS, CORS, authentication, limits,
   uploads, streaming, WebSockets/gRPC, errors, and graceful drain as applicable.
