...
`enterprise-platform` began as a legacy application whose components shared a
release path, runtime assumptions, and operational blast radius. Growth made the
release model harder to scale: a change to one capability could require testing
and releasing the whole application, while vertical resource allocation hid
which capabilities drove memory and compute demand.
